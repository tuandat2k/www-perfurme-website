									...
									<c:if test="${not empty orderFind}">
										<table class="table table-bordered" id="table-catgory">
											...
											<tbody>
													<tr>
														<td>${orderFind.id}</td>
														<td>${orderFind.createdDate}</td>
														<td>${orderFind.userDto.fullName}</td>
														<td>${orderFind.receiver}</td>
														<td>${orderFind.totalMoney}</td>
														<td>${orderFind.status==true ? 'Đã giao':'Chưa giao'}</td>
														
														<td>
														<a class="btn btn-sm btn-primary btn-edit"
																data-toggle="tooltip" title="Cập nhật" href='${edit_order}/${orderFind.id}'><i
																class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>
																
														<button id="btnDelete" idorder="${orderFind.id}" type="button" 
		...
		function deleteorder(data) {
	        $.ajax({
	            ...
	            success: function (result) {
            		window.location.href = "${url_order_chuagiao}?maxPageItem=${maxPageItemProp}&page=${pageProp}";
